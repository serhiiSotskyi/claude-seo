...
| **DataForSEO** | `dataforseo-mcp-server` / Railway remote MCP | Official extension | 9 modules, 79 tools, 22 commands. For Claude Team, use the Railway connector setup in [Claude Team DataForSEO Setup](CLAUDE-TEAM-DATAFORSEO.md). For Claude Code/local development, install with `./extensions/dataforseo/install.sh`. |
