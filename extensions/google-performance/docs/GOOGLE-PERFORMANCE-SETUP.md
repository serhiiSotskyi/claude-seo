...
2. Enable:
   ...
   - Google Ads API
3. Create a Google OAuth client and use it to generate an offline refresh token
   for the authorized Google user. This is separate from Claude connector OAuth.
4. Grant only `analytics.readonly`, `webmasters.readonly`, and `adwords` scopes.
5. Store the Google OAuth client and refresh token in the hosting environment.
...
Use one client per Railway service for the pilot. The connector refuses multiple
clients unless `ALLOW_MULTITENANT=true`; do not enable that override until
tenant-scoped authorization and encrypted per-client token storage exist.
