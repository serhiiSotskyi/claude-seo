...
Pilot deployments are single-client by default. If more than one client is
present in `CLIENT_ALLOWLIST_JSON`, the service refuses to start unless
`ALLOW_MULTITENANT=true`. Do not enable that override until tenant-scoped
authorization and encrypted per-client Google token storage are implemented.
...
Google API access is a separate OAuth relationship from Claude-to-MCP OAuth.
Generate the Google refresh token through an authorized Google OAuth flow with:
...
- Deploy one service per client during the pilot.
- Add tenant-scoped authorization and a persistent encrypted token store before
  consolidating multiple client Google accounts into one service.
