...
const multiTenantEnabled = process.env.ALLOW_MULTITENANT === "true";
if (Object.keys(clients).length > 1 && !multiTenantEnabled) {
  console.error(
    "Multiple clients configured. Use one deployment per client for pilots, or set ALLOW_MULTITENANT=true only after implementing tenant-scoped authorization and encrypted token storage.",
  );
  process.exit(1);
}
...
app.get("/health", (_req, res) => {
  res.status(200).json({
    ...
    deploymentMode: multiTenantEnabled ? "multi-tenant" : "single-client",
    auth: {
